...
def read_file(filename):
    with open(filename,'r') as file:
        sentences = file.readlines() ## returns a list of strings
        packets = []
        for s in sentences: 
            packets.append(s.strip())
...
def extract_messages(packet_list):
    for message in packet_list[0]:
        for alphanum in message:    
            if not(alphanum.isalpha()):
                print(alphanum)
    ...
    # Write your code here
    pass
extract_messages(read_file('input.txt'))
