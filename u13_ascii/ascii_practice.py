...
# upper = chr(rand.randint(65,90))
# lower = chr(rand.randint(97,122))
# special = chr(rand.randint(33,41))
# number = chr(rand.randint(48,57))
...
# for alphanumerical in range(length-3): # aB3xG2#1
#     # generate a random between a certain range and add to password
...
# length = int(input('Enter a length : '))
length = 8
# alphanum = chr(random.randint(33,126))
for alphanum_val in range(length):
    alphanum = chr(random.randint(33,126))
    while not(alphanum in 'l1IO0'):
        password += alphanum
        break
...
for index in range(len(messages)):
    ascii_val = 0 # reset ascii_val to 0
    # print(index)
    for character in messages[index]:
        # print(character)
        ascii_val += ord(character)
        
    checksum.append(ascii_val % 256)
    # print(checksum)
print(f'{messages[0]} --> {checksum[0]}')
print(f'{messages[1]} --> {checksum[1]}')
print(f'{messages[2]} --> {checksum[2]}')
print(f'{messages[3]} --> {checksum[3]}')
print(f'{messages[4]} --> {checksum[4]}')
